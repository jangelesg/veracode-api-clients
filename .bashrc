...
if [ ! -d .venv ];then
  echo "Setting you up for first time use..."
  python3 -m venv .venv
...
if [ ! -f .env ];then
  echo "WARNING: Please create a $HOME/.env file (see README.md for details)."
else
  . ./.env
fi
...
if [ ! -f "$VC_CREDS_FILE" ];then
  echo "WARNING: Please create a $VC_CREDS_FILE file (see README.md for details)."
fi
